-- ==============================================================================
-- FITSPHERE V1 - PERSONAL BEST TIMELINE (PR HISTORY EVENT LOG)
-- Migration: 20260919000001_pr_history.sql
-- Establishes append-only personal record milestone history and updates
-- the authoritative complete_workout_session RPC to persist PR history events.
-- ==============================================================================

-- 1. Create append-only pr_history table
...
-- 3. RLS Policies: Users can read only their own PR history.
-- Direct client INSERT/UPDATE/DELETE is strictly forbidden; writes only via SECURITY DEFINER RPC.
DROP POLICY IF EXISTS "Users can view own PR history" ON public.pr_history;
CREATE POLICY "Users can view own PR history"
    ON public.pr_history FOR SELECT
    USING (user_id = auth.uid());

-- 4. Query Performance Indexes
CREATE INDEX IF NOT EXISTS idx_pr_history_user_exercise 
    ON public.pr_history(user_id, exercise_id, achieved_at DESC);

CREATE INDEX IF NOT EXISTS idx_pr_history_user_date 
    ON public.pr_history(user_id, achieved_at DESC);

-- 5. Authoritative complete_workout_session RPC (Additive PR History Persistence)
...
BEGIN
    ...
    -- Step 5: Authoritative PR Calculations from workout_sets + PR History Logging
    FOR v_set IN
